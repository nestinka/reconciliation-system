...
impl Store {
    ...
    pub async fn seed(&self) -> Result<(), StoreError> {
        ...
        let txns = [
            ("txn-a001","src-acme-bank","BANK-1","2026-05-01",1_000_000i64,"debit","tenant-acme","GBP"),
            ("txn-b001","src-acme-ledger","BANK-1","2026-05-01",1_000_000,"debit","tenant-acme","GBP"),
            ("txn-a002","src-acme-bank","BANK-2","2026-05-01",500_000,"credit","tenant-acme","GBP"),
            ("txn-b002","src-acme-ledger","BANK-2","2026-05-01",500_000,"credit","tenant-acme","GBP"),
            ("txn-a003","src-acme-bank","BANK-3","2026-05-02",250_000,"debit","tenant-acme","GBP"),
            ("txn-b003","src-acme-ledger","BANK-3","2026-05-02",250_000,"debit","tenant-acme","GBP"),
            ("txn-a005","src-acme-bank","BANK-5","2026-05-04",320_000,"debit","tenant-acme","GBP"),
            ("txn-b005","src-acme-ledger","BANK-5","2026-05-04",319_500,"debit","tenant-acme","GBP"),
            ("txn-c004","src-acme-ledger","DUP-9","2026-05-10",95_000,"debit","tenant-acme","GBP"),
            ("txn-c005","src-acme-ledger","DUP-9","2026-05-10",95_000,"debit","tenant-acme","GBP"),
            ("txn-brk001","src-acme-bank","BANK-99","2026-05-15",125_000,"debit","tenant-acme","GBP"),
            ("txn-brk002","src-acme-bank","BANK-10","2026-05-16",67_500,"credit","tenant-acme","GBP"),
            ("txn-brk005","src-acme-bank","BANK-18","2026-05-18",210_000,"debit","tenant-acme","GBP"),
            ("txn-brk006","src-acme-bank","BANK-19","2026-05-23",88_000,"credit","tenant-acme","GBP"),
            ("txn-g001","src-globex-bank","GB-1","2026-05-01",2_000_000,"debit","tenant-globex","USD"),
            ("txn-g002","src-globex-ledger","GB-1","2026-05-01",2_000_000,"debit","tenant-globex","USD"),
            ("txn-g005","src-globex-bank","GB-9","2026-05-10",390_000,"debit","tenant-globex","USD"),
        ];
        ...
        let runs = [
            ("run-acme-001","tenant-acme","Daily Bank-GL 2026-05-05","src-acme-bank","src-acme-ledger","2026-05-05T18:00:00Z","2026-05-01","2026-05-09"),
            ("run-acme-006","tenant-acme","Daily Bank-GL 2026-05-23","src-acme-bank","src-acme-ledger","2026-05-23T18:00:00Z","2026-05-10","2026-05-31"),
            ("run-globex-001","tenant-globex","Globex Daily 2026-05-10","src-globex-bank","src-globex-ledger","2026-05-10T19:00:00Z","2026-05-01","2026-05-31"),
        ];
        ...
        for (run_id, tid, name, sa, sb, started, from, to) in runs {
            ...
            for (i, bd) in result.breaks.iter().enumerate() {
                ...
                let opened: &str = if is_pending { "2026-05-15T10:30:00Z" } else { started };
                let (status, assignee): (&str, Option<&str>) = if is_pending { ("pending_approval", Some("user-mia")) } else { ("open", None) };
                ...
                if is_pending {
                    let evs = [
                        (1i32, "assignment", "user-ada", "2026-05-15T11:00:00Z", serde_json::json!({"assigneeId":"user-mia"})),
    ...
    async fn load_window(&self, tx: &mut sqlx::PgConnection, tenant_id: &str, source_id: &str, from: &str, to: &str) -> Result<Vec<CanonicalTransaction>, StoreError> {
