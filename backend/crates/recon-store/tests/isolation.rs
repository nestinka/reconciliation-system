...
async fn seed_two_tenants(store: &Store) {
    ...
    for (t, name) in [("tenant-a", "Alpha"), ("tenant-b", "Beta")] {
        sqlx::query("INSERT INTO tenants(id,name,slug) VALUES ($1,$2,$1)")
            .bind(t)
            .bind(name)
            .execute(&store.pool)
            .await
        ...
        sqlx::query("INSERT INTO memberships(user_id,tenant_id,role) VALUES ($1,$2,'operator')")
            ...
            .unwrap();
        sqlx::query(
            "INSERT INTO sources(id,tenant_id,kind,name,currency) VALUES ($1,$2,'bank','S','GBP')",
        )
        .bind(format!("s-{t}"))
        .bind(t)
        .execute(&store.pool)
...
async fn tenant_isolation_on_users(pool: sqlx::PgPool) {
    ...
    assert_eq!(
        store
            .list_breaks("tenant-a", &BreakFilter::default())
            .await
            .unwrap()
            .len(),
        0
...
async fn dashboard_counts_open_breaks(pool: sqlx::PgPool) {
    ...
    seed_two_tenants(&store).await;
    sqlx::query(
        "INSERT INTO cases(id,tenant_id,break_id,status) VALUES ('c1','tenant-a','b1','open')",
    )
    .execute(&store.pool)
    .await
    .unwrap();
