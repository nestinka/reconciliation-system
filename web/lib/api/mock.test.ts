...
describe("MockApiClient", () => {
  ...
  it("appendCaseEvent is append-only and does not mutate prior reads", async () => {
    ...
    const next = await c.appendCaseEvent("tenant-acme", "case-pending", { kind: "comment", actorId: "user-sam", payload: { text: "looking into it" } });
    expect(next.events.length).toBe(beforeLen + 1);
    expect(before.events.length).toBe(beforeLen); // earlier read not mutated
  });
  it("assignBreak sets assignee and moves to investigating", async () => {
    const c = api();
